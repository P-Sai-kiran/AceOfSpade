        ...
        {players.map(p => {
          ...
          return (
            <View key={p.seat} style={[s.row, isMe && s.rowMe]}>
              <View style={[s.avatar, isMe && s.avatarMe]}><Text style={s.avatarL}>{p.name?.charAt(0)||"?"}</Text></View>
              <View style={s.mid}>
                <Text style={[s.name, isMe && s.nameMe]}>{isMe?"You":p.name}</Text>
                <Text style={s.detail}>Bid {bid} · Won {won} · {hit?"✓ Hit":over?"↑ Bonus":"✗ Missed"}</Text>
              </View>
              <Text style={[s.pts, pts>=0 ? s.pos : s.neg]}>{pts>=0?"+":""}{pts}</Text>
            </View>
          );
        })}
      </View>
      ...
      <View style={s.standings}>
        {sorted.map((p, i) => (
          <View key={p.seat} style={[s.stand, p.seat===yourSeat && s.standMe]}>
            ...
            <Text style={[s.standName, p.seat===yourSeat&&{color:C.gold}]}>{p.seat===yourSeat?"You":p.name}</Text>
            <Text style={s.total}>{p.totalScore}</Text>
          </View>
        ))}
      </View>
      {round < 13 ? <Text style={s.hint}>Next round starting in a moment…</Text> : <Text style={[s.hint,{color:C.gold}]}>Final round complete!</Text>}
    </ScrollView>
  );
}
