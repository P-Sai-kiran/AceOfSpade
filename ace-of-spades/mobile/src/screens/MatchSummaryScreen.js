    ...
    <ScrollView contentContainerStyle={s.bg}>
      ...
      {ranked.map((p, i) => (
        <View key={p.seat} style={[s.row, p.seat===yourSeat && s.rowMe]}>
          ...
          <View style={[s.avatar, i===0&&s.avatarWin]}>
            <Text style={s.avatarL}>{p.name?.charAt(0)||"?"}</Text>
          </View>
          <Text style={[s.name, p.seat===yourSeat&&{color:C.gold}]}>
        ...
        <View>
          ...
          {history.map(h=>(
            <View key={h.round} style={s.tableRow}>
              ...
              {players.map(p=>{
                const sc=h.scores[p.seat];
                return <Text key={p.seat} style={[s.cell, sc>0?s.pos:sc<0?s.neg:s.zero]}>{sc>0?"+":""}{sc}</Text>;
              })}
            </View>
          ))}
          <View style={[s.tableRow, s.tableTotal]}>
            ...
            {players.map(p=>(
              <Text key={p.seat} style={[s.cell, s.cellHd, p.seat===yourSeat&&{color:C.gold}]}>{p.totalScore}</Text>
            ))}
          </View>
        </View>
      </ScrollView>
      ...
      <View style={s.actions}>
        <TouchableOpacity style={s.lobbyBtn} onPress={onLobby}>
          <Text style={s.lobbyBtnText}>BACK TO HOME</Text>
        </TouchableOpacity>
