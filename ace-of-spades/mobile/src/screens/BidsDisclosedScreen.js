// Brief transitional screen shown for ~3s after the last bid locks in,
// before card play begins. Bids were already visible progressively, so
// this is a short pause/summary rather than a "reveal".
  ...
  return (
    <View style={s.bg}>
      <View style={s.card}>
        ...
        <Text style={s.subtitle}>ALL BIDS IN</Text>
        <View style={s.list}>
          {trickPlayOrder.map(seat => {
            const p = players[seat];
            const isMe = seat === yourSeat;
            const isAdv = seat === advantageSeat;
            return (
              <View key={seat} style={[s.row, isMe && s.rowMe]}>
                <View style={[s.avatar, isAdv && s.avatarAdv]}><Text style={s.avatarLetter}>{p?.name?.charAt(0) || "?"}</Text></View>
                <Text style={[s.name, isMe && s.nameMe]}>{isMe ? "You" : p?.name || `P${seat+1}`}</Text>
        ...
        <View style={s.advBox}>
          ...
          <Text style={s.advText}>{players[advantageSeat]?.name || ""} will play last in every trick</Text>
        </View>
      </View>
    </View>
  );
...
const s = StyleSheet.create({
  ...
  card: { backgroundColor:C.bgCard, borderRadius:R.xl, padding:28, width:"100%", borderWidth:1, borderColor:C.border },
  ...
  row: { flexDirection:"row", alignItems:"center", gap:12, backgroundColor:"rgba(255,255,255,0.04)", borderRadius:R.md, padding:14, borderWidth:1, borderColor:"transparent" },
  ...
  bidMe: { color:C.gold },
  advBox: { flexDirection:"row", alignItems:"center", gap:10, backgroundColor:C.goldBg, borderRadius:R.lg, padding:14, borderWidth:1, borderColor:C.gold },
