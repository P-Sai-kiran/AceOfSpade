...
export default function LobbyScreen({ roomCode, myName }) {
  const { lobbyPlayers, lobbyColor } = useGame();
  const palette = TABLE_COLORS[lobbyColor] || TABLE_COLORS.green;
    ...
    <View style={s.bg}>
      <View style={s.header}>
        ...
        <View style={s.colorRow}>
          <View style={[s.colorDot, { backgroundColor: palette.felt }]} />
          <Text style={s.colorText}>{palette.label} table</Text>
        </View>
      </View>
          ...
          return (
            <View key={i} style={[s.slot, isMe && s.slotMe, filled && s.slotFilled]}>
              ...
              <View style={s.slotInfo}>
                <Text style={[s.slotName, !filled && s.slotNameEmpty]}>{filled ? name : `Waiting for player ${i + 1}…`}</Text>
...
const s = StyleSheet.create({
  ...
  colorRow: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 14 },
  colorDot: { width: 16, height: 16, borderRadius: 8, borderWidth: 2, borderColor: C.gold },
  colorText:{ color: C.textSec, fontSize: F.sm },
  slots:    { gap: 10, marginBottom: 28 },
  slot:     { flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: C.bgCard, borderRadius: R.lg, padding: 14, borderWidth: 1, borderColor: C.border },
  slotMe:     { borderColor: C.gold },
  slotFilled: { borderColor: C.border },
  avatar:     { width: 44, height: 44, borderRadius: 22, backgroundColor: C.border, justifyContent: "center", alignItems: "center" },
