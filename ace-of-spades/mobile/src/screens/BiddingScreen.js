...
// each locked bid is instantly shown to everyone.
...
export default function BiddingScreen() {
  const { state } = useGame();
  const { round=1, yourSeat, players=[], bids={}, currentBidder, trickPlayOrder=[], advantageSeat } = state || {};
...
const s = StyleSheet.create({
  ...
  btn:     { backgroundColor: C.gold, borderRadius: R.lg, paddingVertical: 18, alignItems: "center" },
  btnOff:  { opacity: 0.6 },
  btnText: { color: C.bg, fontWeight: "900", fontSize: F.md, letterSpacing: 2 },
  waiting: { alignItems: "center", backgroundColor: C.bgCard, borderRadius: R.xl, padding: 32, borderWidth: 1, borderColor: C.border },
