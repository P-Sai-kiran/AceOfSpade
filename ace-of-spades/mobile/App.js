...
const S = { SPLASH:"splash", HOME:"home", CREATE:"create", JOIN:"join",
            LOBBY:"lobby", GAME:"game", HOW:"how" };

function ConnectionBanner({ connected }) {
  if (connected) return null;
  return (
    <View style={bs.banner}>
      <Text style={bs.bannerText}>⚠ Connecting to server…</Text>
    </View>
  );
}
...
function Navigator() {
  ...
  // Navigate into game as soon as server sends first state
  useEffect(() => {
    if (!state) return;
    if (!inGame.current) {
      inGame.current = true;
  ...
  // Show shuffle animation on each new round (but not round 1)
  useEffect(() => {
  ...
  // ── In-game routing driven by server phase ──────────────────────────────
  ...
  switch (screen) {
    case S.SPLASH:
      return <SplashScreen onDone={() => setScreen(S.HOME)} />;
    case S.HOME:
      return (
        <HomeScreen
          playerName={myName}
          onCreateRoom={()   => setScreen(S.CREATE)}
          onJoinRoom={()     => setScreen(S.JOIN)}
          onHowToPlay={()    => setScreen(S.HOW)}
        />
      );
    case S.HOW:
      return <HowToPlayScreen onBack={() => setScreen(S.HOME)} />;
    case S.CREATE:
      return (
        <CreateJoinScreen mode="create"
    ...
    case S.JOIN:
      return (
        <CreateJoinScreen mode="join"
          onBack={() => setScreen(S.HOME)}
          onJoined={onJoined}
        />
      );
    case S.LOBBY:
      return <LobbyScreen roomCode={roomCode} myName={myName} />;
    default:
      return <HomeScreen onCreateRoom={() => setScreen(S.CREATE)} onJoinRoom={() => setScreen(S.JOIN)} />;
  }
}
